...
# Download a release of GitLeaks
wget -q -O /tmp/gl.tgz $GITLEAKS_RELEASE_LOCATION
# Setup the set of rules used by GitLeaks
## Use my common set of rules
# wget -q -O /tmp/rules.toml $GITLEAKS_RULES_LOCATION
## Use a local set of rules
cp gitleaks-custom-config.toml /tmp/rules.toml
# Scan
